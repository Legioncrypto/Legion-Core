...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("LegionMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("LegionMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("LegionMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("legion-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running LegionMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
