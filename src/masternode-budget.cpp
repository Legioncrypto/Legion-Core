...
int GetBudgetPaymentCycleBlocks()
{
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN)
        return ((24 * 60 * 60) / 90) * 30;

    //for testing purposes
    return ((24 * 60 * 60) / 90) / 10; // ten times per day
}
...
CAmount CBudgetManager::GetTotalBudget(int nHeight)
{
    ...
    if (nHeight > Params().LAST_POW_BLOCK() && nHeight <= 11000)
        nSubsidy = 2.5 * COIN;
    else if (nHeight > 11000 && nHeight <= 21000)
        nSubsidy = 1.0 * COIN;
    else if (nHeight > 21000 && nHeight <= 31000)
        nSubsidy = 0.5 * COIN;
    else
        nSubsidy = 0.2 * COIN;

    // Amount of blocks in a months period of time
    return ((nSubsidy / 100) * 10) * ((24 * 60 * 60) / 90) * 30;
}
