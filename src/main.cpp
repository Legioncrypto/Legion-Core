...
#if defined(NDEBUG)
#error "Legion cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ulegn) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    int64_t nSubsidy = 0;
     ...
     if (nHeight == 1)
        nSubsidy = 30000000* COIN; // 2.5M for pre-mine
    if (nHeight == 1)
        nSubsidy = 30000000* COIN; // 30.0M for pre-mine for swap
    else if (nHeight <= Params().LAST_POW_BLOCK()) //phase 1 of chain build up Block 2-1050
        nSubsidy = 40 * COIN;
    else if (nHeight > Params().LAST_POW_BLOCK() && nHeight <= 5000) //phase 2 of chain build up Block 1051-5000 Staking and MasterNodes payments kick in
        nSubsidy = 40 * COIN;
